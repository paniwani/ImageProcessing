...
int main( int argc, char *argv[] )
{
	if ( argc < 2 ) {
		...
		std::cerr << "HessianTest3 input outputImage" << std::endl;
		system("PAUSE");
		return EXIT_FAILURE;
	}
	...
	myfile << "HessianTest3\n";
	myfile << "sigma," << sigma << "\nalpha," << alpha << "\nbeta," << beta << "\ngamma," << gamma << "\n\n";
	myfile << "index_x,index_y,index_z,lambda1,lambda2,lambda3,fa\n";
	...
	*/
			...
			++input_iter, ++tensor_iter, ++F_rut_iter, ++F_a_iter, ++F_b_iter)
	{
		...
		if (input_iter.Get() == 40) { //input is colon mask, look only inside
			...
			if (ev_vector[2] > .00001) {
				//Solve functions
				//F_rut_iter.Set(f_rut(ev_vector, alpha, beta, gamma));
				//F_cup_iter.Set(f_cup(ev_vector, eta));

				//Write index
				ImageType::IndexType idx = F_a_iter.GetIndex();
				myfile << idx[0] << "," << idx[1] << "," << idx[2] << ",";
				
				//Write eigenvalues
				myfile << ev_vector[0] << "," << ev_vector[1] << "," << ev_vector[2] << ",";
				
				//Write output
				double fa = functionA(ev_vector,alpha);
				myfile << fa << "\n";
				F_a_iter.Set(fa);
			} else {
				F_a_iter.Set(0);
	...
	*/
	...
	std::stringstream ss;
	ss << "F_a" << "_sigma" << sigma << "_alpha " << alpha << ".hdr";
	writer->SetFileName(ss.str().c_str());
	writer->Update();
