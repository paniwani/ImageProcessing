...
=========================================================================*/
#ifndef __itkCannyEdgeDetectionImageFilterModified_txx
#define __itkCannyEdgeDetectionImageFilterModified_txx
...
CannyEdgeDetectionImageFilterModified<TInputImage, TOutputImage>::
...
template< class TInputImage, class TOutputImage >
typename CannyEdgeDetectionImageFilterModified< TInputImage, TOutputImage >
...
::GenerateData()
{
  ...
  std::cout << m_LowerThreshold << std::endl;
  
  typename OutputImageType::Pointer canny = m_MultiplyImageFilter->GetOutput();
  
  ImageRegionIterator<TOutputImage> cit( canny, canny->GetRequestedRegion() );
  ImageRegionIterator<TOutputImage> uit(this->GetOutput(),this->GetOutput()->GetRequestedRegion());
  
  for (cit.GoToBegin(), uit.GoToBegin(); !cit.IsAtEnd(); ++cit, ++uit)
  {
	if (cit.Get() > m_LowerThreshold )
	{
		uit.Set( cit.Get() );
	} else {
		uit.Set( 0 );
